...
func (ipm *InfoPanelManager) UpdatePanels(infoPanelVMs []core.InfoPanelViewModel, mainUIContainer *widget.Container, battlefieldRect image.Rectangle, iconVMs []*core.IconViewModel) {
	// 既存のパネルをクリア
...
func createSingleMedarotInfoPanel(config *data.Config, uiFactory *UIFactory, vm core.InfoPanelViewModel) *infoPanelUI {
	c := config.UI
...
func updateSingleInfoPanel(ui *infoPanelUI, vm core.InfoPanelViewModel, config *data.Config) {
	c := config.UI
