...
*   `ui/battle_ui_manager.go`: **[ロジック/振る舞い]** バトルUI全体の司令塔。UIの初期化、更新、描画、イベント処理など、すべてのUI関連ロジックを統括します。各種サブマネージャ（`InfoPanelManager`, `ActionModalManager`など）を内包し、UIのライフサイクル全体を管理します。
*   `ui/ui.go`
    *   役割: **（リファクタリングにより内容は空）**
    *   内容: かつてUI全体の管理を担っていましたが、責務が`BattleUIManager`に集約されたため、ファイルの内容は空になりました。レイアウト関連のヘルパー関数は`ui/ui_layout.go`に移動しました。
