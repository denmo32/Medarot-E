...
*   `ui/battle_ui_manager.go`: **[ロジック/振る舞い]** バトルUI全体の司令塔。UIの初期化、更新、描画、およびアクションモーダル、メッセージウィンドウ、ターゲットインジケーターといった各UI要素の状態管理をすべて統括します。`BattleScene`から渡されたViewModelに基づいてUIの状態を更新し、UIウィジェットからの入力をゲームイベントとして`BattleScene`に通知します。
