...
*   `battle_action_queue_system.go`: **[ロジック/振る舞い]** 行動実行キューを処理し、適切な `ActionHandler` を呼び出してアクションを実行します。
*   `battle_action_handler.go`: **[ロジック/振る舞い]** パーツカテゴリ別（射撃、格闘など）の具体的なアクション実行ロジックをカプセル化します。`ActionHandler` インターフェースとその実装（`ShootActionHandler`, `MeleeActionHandler`など）を定義します。
*   `battle_action_processor.go`: **[ロジック/振る舞い]** アクションの実行結果（ダメージ計算、デバフ適用など）を処理します。
*   `battle_logic_end_system.go`: **[ロジック/振る舞い]** ゲーム終了条件判定システム。`CheckGameEndSystem` を定義します。
*   `battle_logic_gauge_system.go`: **[ロジック/振る舞い]** チャージゲージおよびクールダウンゲージの進行管理システム。`UpdateGaugeSystem` を定義します。
...
*   `battle_action_formulas.go`: **[データ]** アクションの計算式定義と管理。
