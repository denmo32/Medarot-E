...
func createSingleMedarotInfoPanel(config *game.Config, uiFactory *UIFactory, vm InfoPanelViewModel) *infoPanelUI {
	...
	partSlots := make(map[game.PartSlotKey]*infoPanelPartUI)
	for _, slotKey := range []game.PartSlotKey{game.PartSlotHead, game.PartSlotRightArm, game.PartSlotLeftArm, game.PartSlotLegs} {
...
func CreateInfoPanels(config *game.Config, uiFactory *UIFactory, infoPanelVMs []InfoPanelViewModel) []InfoPanelCreationResult {
...
func updateSingleInfoPanel(ui *infoPanelUI, vm InfoPanelViewModel, config *game.Config) {
	c := config.UI.(UIConfig)
	...
	for slotKey, partUI := range ui.partSlots {
		...
		if partUI.displayedHP < partUI.targetHP {
			partUI.displayedHP += hpAnimationSpeed
			if partUI.displayedHP > partUI.targetHP {
