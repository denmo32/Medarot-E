...
func NewUIAnimationDrawer(config *game.Config, animationManager game.AnimationManager, gdm *game.GameDataManager) *UIAnimationDrawer {
...
func (d *UIAnimationDrawer) Draw(screen *ebiten.Image, tick int, battlefieldVM BattlefieldViewModel, battlefieldWidget *BattlefieldWidget) {
	if d.animationManager.IsAnimationFinished(tick) {
		return
	}
	anim := d.animationManager.GetCurrentAnimationResult()
	...
	for _, icon := range battlefieldVM.Icons {
		...
		if anim.TargetEntry != nil && icon.EntryID == uint32(anim.TargetEntry.Id()) {
			targetVM = icon
		}
	}
	...
	if attackerVM != nil && targetVM != nil {
		...
		if popupStartProgress >= 0 {
			...
			text.Draw(screen, fmt.Sprintf("-%d", anim.OriginalDamage), d.gameDataManager.Font, drawOpts)
		}
	}
}
