...
type UIActionModalManager struct {
	...
	actionTargetMap      map[game.PartSlotKey]game.ActionTarget // 選択可能なアクションとターゲットのマップ
	eventChannel         chan game.UIEvent                    // UIイベント通知用
	uiFactory            *UIFactory                           // 追加
}

// NewUIActionModalManager は新しいUIActionModalManagerのインスタンスを作成します。
func NewUIActionModalManager(ebitenui *ebitenui.UI, eventChannel chan game.UIEvent, uiFactory *UIFactory) *UIActionModalManager {
	return &UIActionModalManager{
		ebitenui:             ebitenui,
		isActionModalVisible: false,
		actionTargetMap:      make(map[game.PartSlotKey]game.ActionTarget),
...
func (m *UIActionModalManager) ShowActionModal(vm ActionModalViewModel) {
	...
	m.actionTargetMap = make(map[game.PartSlotKey]game.ActionTarget)
	for _, btn := range vm.Buttons {
		m.actionTargetMap[btn.SlotKey] = game.ActionTarget{Target: btn.TargetEntry, Slot: btn.SlotKey}
	}
...
func (m *UIActionModalManager) HideActionModal() {
	...
	m.isActionModalVisible = false
	m.actionTargetMap = make(map[game.PartSlotKey]game.ActionTarget) // ターゲットマップをクリア
...
func (m *UIActionModalManager) GetActionTargetMap() map[game.PartSlotKey]game.ActionTarget {
