...
func CheckGameEndSystem(world donburi.World) GameEndResult {
	team1Leader := FindLeader(world, game.Team1) // FindLeader は ecs_setup.go にあります
	team2Leader := FindLeader(world, game.Team2) // FindLeader は ecs_setup.go にあります
	...
	query.NewQuery(filter.Contains(game.SettingsComponent)).Each(world, func(entry *donburi.Entry) {
		if !game.StateComponent.Get(entry).FSM.Is(string(game.StateBroken)) {
			if game.SettingsComponent.Get(entry).Team == game.Team1 {
				team1FuncCount++
			} else {
				team2FuncCount++
			}
	...
	if team1Leader == nil || game.PartsComponent.Get(team1Leader).Map[game.PartSlotHead].IsBroken || team1FuncCount == 0 {
		winner = game.Team2
		isGameOver = true
		if team1Leader != nil && game.PartsComponent.Get(team1Leader).Map[game.PartSlotHead].IsBroken {
			gameOverMsg = fmt.Sprintf("%sが機能停止！ チーム2の勝利！", game.SettingsComponent.Get(team1Leader).Name)
	...
	if !isGameOver { // チーム1がまだ敗北していない場合のみチーム2の敗北をチェック
		if team2Leader == nil || game.PartsComponent.Get(team2Leader).Map[game.PartSlotHead].IsBroken || team2FuncCount == 0 {
			winner = game.Team1
			isGameOver = true
			if team2Leader != nil && game.PartsComponent.Get(team2Leader).Map[game.PartSlotHead].IsBroken {
				gameOverMsg = fmt.Sprintf("%sが機能停止！ チーム1の勝利！", game.SettingsComponent.Get(team2Leader).Name)
...
func FindLeader(world donburi.World, teamID game.TeamID) *donburi.Entry {
