...
type PartInfoProvider struct {
	world           donburi.World
	config          *game.Config
	gameDataManager *game.GameDataManager // GlobalGameDataManagerを注入
}

// NewPartInfoProvider は新しい PartInfoProvider のインスタンスを生成します。
func NewPartInfoProvider(world donburi.World, config *game.Config, gdm *game.GameDataManager) *PartInfoProvider {
...
func (pip *PartInfoProvider) GetPartParameterValue(entry *donburi.Entry, partSlot game.PartSlotKey, param game.PartParameter) float64 {
...
func (pip *PartInfoProvider) FindPartSlot(entry *donburi.Entry, partToFindInstance *game.PartInstanceData) game.PartSlotKey {
	partsComp := game.PartsComponent.Get(entry)
...
type AvailablePart struct {
	PartDef  *game.PartDefinition // Changed from Part to PartDefinition
	Slot     game.PartSlotKey
...
func (pip *PartInfoProvider) GetAvailableAttackParts(entry *donburi.Entry) []AvailablePart {
	...
	slotsToConsider := []game.PartSlotKey{game.PartSlotHead, game.PartSlotRightArm, game.PartSlotLeftArm}
	...
	for _, slot := range slotsToConsider {
		...
		if partDef.Category == game.CategoryRanged || partDef.Category == game.CategoryMelee || partDef.Category == game.CategoryIntervention {
...
func (pip *PartInfoProvider) GetOverallPropulsion(entry *donburi.Entry) int {
	return int(pip.GetPartParameterValue(entry, game.PartSlotLegs, game.Propulsion))
}

// GetOverallMobility はエンティティの総機動力を返します。
func (pip *PartInfoProvider) GetOverallMobility(entry *donburi.Entry) int {
	return int(pip.GetPartParameterValue(entry, game.PartSlotLegs, game.Mobility))
}

// GetLegsPartDefinition はエンティティの脚部パーツの定義を取得します。
func (pip *PartInfoProvider) GetLegsPartDefinition(entry *donburi.Entry) (*game.PartDefinition, bool) {
	partsComp := game.PartsComponent.Get(entry)
	if partsComp == nil {
		return nil, false
	}
	legsInstance, ok := partsComp.Map[game.PartSlotLegs]
...
func (pip *PartInfoProvider) GetSuccessRate(entry *donburi.Entry, actingPartDef *game.PartDefinition, selectedPartKey game.PartSlotKey) float64 {
...
func (pip *PartInfoProvider) GetEvasionRate(entry *donburi.Entry) float64 {
	evasion := pip.GetPartParameterValue(entry, game.PartSlotLegs, game.Mobility)
...
func (pip *PartInfoProvider) GetDefenseRate(entry *donburi.Entry) float64 {
	defense := pip.GetPartParameterValue(entry, game.PartSlotLegs, game.Defense)
...
func (pip *PartInfoProvider) GetTeamAccuracyBuffMultiplier(entry *donburi.Entry) float64 {
	teamBuffsEntry, ok := query.NewQuery(filter.Contains(game.TeamBuffsComponent)).First(pip.world)
	if !ok {
		return 1.0 // バフコンポーネントがなければ効果なし
	}
	teamBuffs := game.TeamBuffsComponent.Get(teamBuffsEntry)
	settings := game.SettingsComponent.Get(entry)

	teamID := settings.Team
	buffType := game.BuffTypeAccuracy
...
func (pip *PartInfoProvider) RemoveBuffsFromSource(entry *donburi.Entry, partInst *game.PartInstanceData) {
	teamBuffsEntry, ok := query.NewQuery(filter.Contains(game.TeamBuffsComponent)).First(pip.world)
	if !ok {
		return
	}
	...
	for teamID, buffMap := range teamBuffs.Buffs {
		for buffType, buffSources := range buffMap {
			newBuffSources := make([]*game.BuffSource, 0, len(buffSources))
			for _, buff := range buffSources {
				// このパーツからのバフでなければ保持する
...
func (pip *PartInfoProvider) CalculateGaugeDuration(baseSeconds float64, entry *donburi.Entry) float64 {
	...
	if entry != nil && entry.HasComponent(game.PartsComponent) {
		partsComp := game.PartsComponent.Get(entry)
		if partsComp != nil {
			legsInstance, ok := partsComp.Map[game.PartSlotLegs]
			if ok && legsInstance != nil && !legsInstance.IsBroken {
...
func (pip *PartInfoProvider) CalculateMedarotXPosition(entry *donburi.Entry, battlefieldWidth float32) float32 {
	...
	if settings.Team == game.Team2 {
		homeX, execX = battlefieldWidth*0.9, battlefieldWidth*0.6
	}

	var xPos float32
	if state.FSM.Is(string(game.StateCharging)) {
		xPos = homeX + (execX-homeX)*progress
	} else if state.FSM.Is(string(game.StateReady)) {
		xPos = execX
	} else if state.FSM.Is(string(game.StateCooldown)) {
		xPos = execX - (execX-homeX)*progress
	} else if state.FSM.Is(string(game.StateIdle)) || state.FSM.Is(string(game.StateBroken)) {
		xPos = homeX
	} else {
