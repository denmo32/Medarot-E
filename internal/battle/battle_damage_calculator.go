...
func NewDamageCalculator(world donburi.World, config *game.Config) *DamageCalculator {
...
func (dc *DamageCalculator) ApplyDamage(entry *donburi.Entry, partInst *game.PartInstanceData, damage int, battleLogic *BattleLogic) {
	if damage < 0 {
		damage = 0
	}
...
func (dc *DamageCalculator) CalculateDamage(attacker, target *donburi.Entry, actingPartDef *game.PartDefinition, selectedPartKey game.PartSlotKey, battleLogic *BattleLogic) (int, bool) {
	// 1. 計算式の取得
	formula, ok := FormulaManager[actingPartDef.Trait]
	if !ok {
		log.Printf("警告: 特性 '%s' に対応する計算式が見つかりません。デフォルトを使用します。", actingPartDef.Trait)
		formula = FormulaManager[game.TraitShoot]
	}
	...
	if globalRand.Intn(100) < int(criticalChance) {
		isCritical = true
		log.Printf("%s の攻撃がクリティカル！ (確率: %.1f%%)", game.SettingsComponent.Get(attacker).Name, criticalChance)
		// クリティカル時は回避度を0にする
		evasion = 0
	}
...
func (dc *DamageCalculator) GenerateActionLog(attacker, target *donburi.Entry, actingPartDef *game.PartDefinition, targetPartDef *game.PartDefinition, damage int, isCritical bool, didHit bool, battleLogic *BattleLogic) string {
	attackerSettings := game.SettingsComponent.Get(attacker)
	targetSettings := game.SettingsComponent.Get(target)
	skillName := "(不明なスキル)"
...
func (dc *DamageCalculator) CalculateReducedDamage(originalDamage int, targetEntry *donburi.Entry, battleLogic *BattleLogic) int {
	...
	defenseValue := battleLogic.GetPartInfoProvider().GetPartParameterValue(targetEntry, game.PartSlotLegs, game.Defense)
...
func (dc *DamageCalculator) GenerateActionLogDefense(target *donburi.Entry, defensePartDef *game.PartDefinition, damageDealt int, originalDamage int, isCritical bool, battleLogic *BattleLogic) string {
	targetSettings := game.SettingsComponent.Get(target)
