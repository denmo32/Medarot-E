...
type TraitActionHandler interface {
	Execute(
		...
		actingPartDef *game.PartDefinition,
	) ActionResult
}
...
func initializeAttackResult(actingEntry *donburi.Entry, actingPartDef *game.PartDefinition, battleLogic *BattleLogic) ActionResult {
	_ = battleLogic // リンターの未使用パラメータ警告を抑制
	return ActionResult{
		ActingEntry:    actingEntry,
		ActionDidHit:   false, // 初期値はfalse
		AttackerName:   game.SettingsComponent.Get(actingEntry).Name,
...
func (h *BaseAttackHandler) performAttackLogic(
	...
	result.DefenderName = game.SettingsComponent.Get(targetEntry).Name
...
func validateTarget(targetEntry *donburi.Entry, targetPartSlot game.PartSlotKey) bool {
	if game.StateComponent.Get(targetEntry).FSM.Is(string(game.StateBroken)) {
		return false
	}
	targetParts := game.PartsComponent.Get(targetEntry)
...
func performHitCheck(actingEntry, targetEntry *donburi.Entry, actingPartDef *game.PartDefinition, selectedPartKey game.PartSlotKey, battleLogic *BattleLogic) bool {
...
func applyDamageAndDefense(
	...
	actingEntry *donburi.Entry,
	actingPartDef *game.PartDefinition,
	selectedPartKey game.PartSlotKey,
	battleLogic *BattleLogic,
) {
	...
	if defendingPartInst != nil && battleLogic.GetHitCalculator().CalculateDefense(actingEntry, result.TargetEntry, actingPartDef, selectedPartKey, battleLogic) {
		...
		intendedTargetPartInstance := game.PartsComponent.Get(result.TargetEntry).Map[result.TargetPartSlot]
...
func finalizeActionResult(result *ActionResult, battleLogic *BattleLogic) {
	actualHitPartInst := game.PartsComponent.Get(result.TargetEntry).Map[result.ActualHitPartSlot]
...
func resolveAttackTarget(
	actingEntry *donburi.Entry,
	battleLogic *BattleLogic,
) (targetEntry *donburi.Entry, targetPartSlot game.PartSlotKey) {
	targetComp := game.TargetComponent.Get(actingEntry)
	switch targetComp.Policy {
	case game.PolicyPreselected:
	...
	case game.PolicyClosestAtExecution:
...
func (h *SupportTraitExecutor) Execute(
	...
	teamBuffsEntry, ok := query.NewQuery(filter.Contains(game.TeamBuffsComponent)).First(world)
	if !ok {
	...
	if _, exists := teamBuffs.Buffs[teamID]; !exists {
		teamBuffs.Buffs[teamID] = make(map[game.BuffType][]*game.BuffSource)
	}
	if _, exists := teamBuffs.Buffs[teamID][buffType]; !exists {
		teamBuffs.Buffs[teamID][buffType] = make([]*game.BuffSource, 0)
	}

	existingBuffs := teamBuffs.Buffs[teamID][buffType]
	filteredBuffs := make([]*game.BuffSource, 0, len(existingBuffs))
