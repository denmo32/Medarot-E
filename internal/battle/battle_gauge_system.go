...
func UpdateGaugeSystem(world donburi.World) {
	query.NewQuery(filter.Contains(game.StateComponent)).Each(world, func(entry *donburi.Entry) {
		state := game.StateComponent.Get(entry)

		// チャージ中またはクールダウン中のエンティティのみを処理
		if !state.FSM.Is(string(game.StateCharging)) && !state.FSM.Is(string(game.StateCooldown)) {
			return
		}
		...
		if gauge.ProgressCounter >= gauge.TotalDuration {
			...
			if state.FSM.Is(string(game.StateCharging)) {
				...
				if err != nil {
					log.Printf("Error transitioning to ready state for %s: %v", game.SettingsComponent.Get(entry).Name, err)
				}
				actionQueueComp := GetActionQueueComponent(world)
				actionQueueComp.Queue = append(actionQueueComp.Queue, entry)
				log.Printf("%s のチャージが完了。実行キューに追加。", game.SettingsComponent.Get(entry).Name)
			} else if state.FSM.Is(string(game.StateCooldown)) {
				err := state.FSM.Event(ctx, "cooldown_finish", entry)
				if err != nil {
					log.Printf("Error transitioning to idle state for %s: %v", game.SettingsComponent.Get(entry).Name, err)
				}
			}
		}
	})
