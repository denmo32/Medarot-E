...
func (s *PostActionEffectSystem) Process(result *ActionResult) {
	...
	// 2. パーツ破壊による状態遷移
	if result.TargetEntry != nil && result.TargetPartBroken && result.ActualHitPartSlot == game.PartSlotHead {
		state := game.StateComponent.Get(result.TargetEntry)
		if state.FSM.Can("break") {
			err := state.FSM.Event(context.Background(), "break", result.TargetEntry)
			if err != nil {
				log.Printf("Error breaking medarot %s: %v", game.SettingsComponent.Get(result.TargetEntry).Name, err)
			}
		}
	}
	...
	if result.ActingEntry != nil && result.ActingEntry.HasComponent(game.ActiveEffectsComponent) {
		activeEffects := game.ActiveEffectsComponent.Get(result.ActingEntry)
		// このループ内でRemoveを呼ぶとスライスが変更されて危険なので、
		// 解除すべきエフェクトを一旦リストアップする
		effectsToRemove := []game.StatusEffect{}
