...
) (game.PartSlotKey, *game.PartDefinition) // 選択されたパーツのスロットキーとその定義を返します。
...
func aiSelectAction(
	...
	var slotKey game.PartSlotKey
	var selectedPartDef *game.PartDefinition
	...
	if entry.HasComponent(game.AIComponent) {
		ai := game.AIComponent.Get(entry)
...
func SelectFirstAvailablePart(
	...
	world donburi.World,
	battleLogic *BattleLogic, // battleLogic を追加
) (game.PartSlotKey, *game.PartDefinition) {
	if len(availableParts) > 0 {
