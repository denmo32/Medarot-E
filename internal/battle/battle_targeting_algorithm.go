...
type TargetingStrategy interface {
	...
	) (*donburi.Entry, game.PartSlotKey)
}
...
type targetablePart struct {
	...
	partInst *game.PartInstanceData
	partDef  *game.PartDefinition
...
func getAllTargetableParts(actingEntry *donburi.Entry, battleLogic *BattleLogic, includeHead bool) []targetablePart {
	...
	for _, enemyEntry := range candidates {
		partsComp := game.PartsComponent.Get(enemyEntry)
		if partsComp == nil {
			continue
		}
		for slotKey, partInst := range partsComp.Map {
			...
			if !includeHead && partDef.Type == game.PartTypeHead {
				continue
			}
...
func (s *LeaderStrategy) SelectTarget(
	...
	leader := game.FindLeader(world, opponentTeamID)

	if leader != nil && !game.StateComponent.Get(leader).FSM.Is(string(game.StateBroken)) {
...
func (s *ChaseStrategy) SelectTarget(
	...
	for _, p := range targetParts {
		if p.partDef.Type == game.PartTypeLegs {
...
func (s *DuelStrategy) SelectTarget(
	...
	for _, p := range targetParts {
		if (p.partDef.Type == game.PartTypeLArm || p.partDef.Type == game.PartTypeRArm) &&
			(p.partDef.Category == game.CategoryRanged || p.partDef.Category == game.CategoryMelee) {
...
func (s *InterceptStrategy) SelectTarget(
	...
	for _, p := range targetParts {
		if p.partDef.Category != game.CategoryRanged && p.partDef.Category != game.CategoryMelee {
...
func (s *CounterStrategy) SelectTarget(
	...
	if actingEntry.HasComponent(game.AIComponent) {
		ai := game.AIComponent.Get(actingEntry)
		if ai.TargetHistory.LastAttacker != nil {
			lastAttacker := ai.TargetHistory.LastAttacker
			// 攻撃者がまだ有効で、破壊されていないことを確認
			if lastAttacker.Valid() && !game.StateComponent.Get(lastAttacker).FSM.Is(string(game.StateBroken)) {
				...
				if targetPart != nil {
					...
					if slotKey != "" {
						log.Printf("AI戦略 [カウンター]: %s が最後に攻撃してきた %s を狙います。", game.SettingsComponent.Get(actingEntry).Name, game.SettingsComponent.Get(lastAttacker).Name)
...
func (s *GuardStrategy) SelectTarget(
	...
	leader := game.FindLeader(world, settings.Team)

	if leader != nil && leader != actingEntry { // リーダーがいて、自分自身ではない場合
		if leader.HasComponent(game.AIComponent) {
			ai := game.AIComponent.Get(leader)
			if ai.TargetHistory.LastAttacker != nil {
				lastAttacker := ai.TargetHistory.LastAttacker
				if lastAttacker.Valid() && !game.StateComponent.Get(lastAttacker).FSM.Is(string(game.StateBroken)) {
					targetPart := battleLogic.GetTargetSelector().SelectPartToDamage(lastAttacker, actingEntry, battleLogic) // battleLogic を追加
					if targetPart != nil {
						slotKey := battleLogic.GetPartInfoProvider().FindPartSlot(lastAttacker, targetPart) // partInfoProvider を battleLogic から取得
						if slotKey != "" {
							log.Printf("AI戦略 [ガード]: %s がリーダーを攻撃した %s を狙います。", game.SettingsComponent.Get(actingEntry).Name, game.SettingsComponent.Get(lastAttacker).Name)
							return lastAttacker, slotKey
						}
					}
				}
...
func (s *FocusStrategy) SelectTarget(
	...
	if actingEntry.HasComponent(game.AIComponent) {
		...
		if ai.LastActionHistory.LastHitTarget != nil && ai.LastActionHistory.LastHitPartSlot != "" {
			...
			if lastTarget.Valid() && !game.StateComponent.Get(lastTarget).FSM.Is(string(game.StateBroken)) {
				if parts := game.PartsComponent.Get(lastTarget); parts != nil {
					if partInst, ok := parts.Map[lastSlot]; ok && !partInst.IsBroken {
						log.Printf("AI戦略 [フォーカス]: %s が前回攻撃した %s の %s を再度狙います。", game.SettingsComponent.Get(actingEntry).Name, game.SettingsComponent.Get(lastTarget).Name, lastSlot)
...
func (s *AssistStrategy) SelectTarget(
	...
	actingSettings := game.SettingsComponent.Get(actingEntry)
	var assistTarget *donburi.Entry
	var assistSlot game.PartSlotKey

	// 自分以外の味方をクエリ
	query.NewQuery(filter.Contains(game.SettingsComponent)).Each(world, func(teammate *donburi.Entry) {
		...
		teammateSettings := game.SettingsComponent.Get(teammate)
		if teammateSettings.Team == actingSettings.Team {
			if teammate.HasComponent(game.AIComponent) {
				ai := game.AIComponent.Get(teammate)
				if ai.LastActionHistory.LastHitTarget != nil && ai.LastActionHistory.LastHitPartSlot != "" {
					lastTarget := ai.LastActionHistory.LastHitTarget
					lastSlot := ai.LastActionHistory.LastHitPartSlot
					if lastTarget.Valid() && !game.StateComponent.Get(lastTarget).FSM.Is(string(game.StateBroken)) {
	...
	if assistTarget != nil {
		log.Printf("AI戦略 [アシスト]: %s が味方の攻撃に続き %s の %s を狙います。", actingSettings.Name, game.SettingsComponent.Get(assistTarget).Name, assistSlot)
