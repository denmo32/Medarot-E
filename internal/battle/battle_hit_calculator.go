...
func NewHitCalculator(world donburi.World, config *game.Config) *HitCalculator {
...
func (hc *HitCalculator) CalculateHit(attacker, target *donburi.Entry, partDef *game.PartDefinition, selectedPartKey game.PartSlotKey, battleLogic *BattleLogic) bool {
	// 攻撃側の成功度
	...
	log.Print(battleLogic.GetPartInfoProvider().gameDataManager.Messages.FormatMessage("log_hit_roll", map[string]interface{}{
		"ordered_args": []interface{}{game.SettingsComponent.Get(attacker).Name, game.SettingsComponent.Get(target).Name, chance, successRate, evasion, roll},
	}))
	return float64(roll) < chance
}

// CalculateDefense は防御の成否を判定します。
func (hc *HitCalculator) CalculateDefense(attacker, target *donburi.Entry, actingPartDef *game.PartDefinition, selectedPartKey game.PartSlotKey, battleLogic *BattleLogic) bool {
	// 攻撃側の成功度
	...
	log.Print(battleLogic.GetPartInfoProvider().gameDataManager.Messages.FormatMessage("log_defense_roll", map[string]interface{}{
		"ordered_args": []interface{}{game.SettingsComponent.Get(target).Name, defenseRate, successRate, chance, roll},
	}))
	return float64(roll) < chance
}
