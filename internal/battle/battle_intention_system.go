...
func UpdatePlayerInputSystem(world donburi.World) PlayerInputSystemResult {
	...
	query.NewQuery(filter.Contains(game.PlayerControlComponent)).Each(world, func(entry *donburi.Entry) {
		if game.StateComponent.Get(entry).FSM.Is(string(game.StateIdle)) {
...
func UpdateAIInputSystem(
	...
	query.NewQuery(
		filter.Not(filter.Contains(game.PlayerControlComponent)), // プレイヤー制御ではないエンティティ
	).Each(world, func(entry *donburi.Entry) {
		if !entry.HasComponent(game.StateComponent) || !game.StateComponent.Get(entry).FSM.Is(string(game.StateIdle)) {
			return
		}
