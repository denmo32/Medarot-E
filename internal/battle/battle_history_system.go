...
func UpdateHistorySystem(world donburi.World, result *ActionResult) {
	if result == nil {
		return
	}
	...
	if result.ActingEntry != nil && result.ActingEntry.Valid() && result.ActingEntry.HasComponent(game.AIComponent) {
		if result.ActionDidHit {
			ai := game.AIComponent.Get(result.ActingEntry)
			...
			log.Printf(
				...
				game.SettingsComponent.Get(result.ActingEntry).Name,
				game.SettingsComponent.Get(result.TargetEntry).Name,
	...
	if result.TargetEntry != nil && result.TargetEntry.Valid() && result.TargetEntry.HasComponent(game.AIComponent) {
		// 命中したかどうかに関わらず、攻撃してきた相手として記録します。
		// これにより、回避した場合でもカウンターの対象となります。
		ai := game.AIComponent.Get(result.TargetEntry)
