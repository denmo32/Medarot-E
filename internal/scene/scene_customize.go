...
type CustomizeScene struct {
	...
	playerMedarots            []*game.MedarotData
	currentTargetMedarotIndex int

	medalList     []*game.Medal
	headPartsList []*game.PartDefinition
	rArmPartsList []*game.PartDefinition
	lArmPartsList []*game.PartDefinition
	legsPartsList []*game.PartDefinition
...
func NewCustomizeScene(res *SharedResources, manager SceneManagerChanger) *CustomizeScene {
	cs := &CustomizeScene{
		resources: res,
		manager:   manager,
	}
...
func (cs *CustomizeScene) setupPartLists() {
	...
	cs.headPartsList = []*game.PartDefinition{}
	cs.rArmPartsList = []*game.PartDefinition{}
	cs.lArmPartsList = []*game.PartDefinition{}
	cs.legsPartsList = []*game.PartDefinition{}

	for _, pDef := range allPartDefs {
		switch pDef.Type {
		case game.PartTypeHead:
			cs.headPartsList = append(cs.headPartsList, pDef)
		case game.PartTypeRArm:
			cs.rArmPartsList = append(cs.rArmPartsList, pDef)
		case game.PartTypeLArm:
			cs.lArmPartsList = append(cs.lArmPartsList, pDef)
		case game.PartTypeLegs:
...
func (cs *CustomizeScene) createLayout() *widget.Container {
	...
	buttonImage := &widget.ButtonImage{
		Idle:    image.NewNineSliceColor(cs.resources.UIConfig.Colors.Gray),
	...
	saveButton := widget.NewButton(
		...
		widget.ButtonOpts.ClickedHandler(func(args *widget.ButtonClickedEventArgs) {
			err := game.SaveMedarotLoadouts("assets/data/medarots.csv", cs.resources.GameData.Medarots)
			if err != nil {
...
func (cs *CustomizeScene) refreshUIForSelectedMedarot() {
	...
	cs.currentMedalIndex = findIndexByIDGeneric(cs.medalList, target.MedalID, func(m *game.Medal) string { return m.ID })
	cs.currentHeadIndex = findIndexByIDGeneric(cs.headPartsList, target.HeadID, func(p *game.PartDefinition) string { return p.ID })
	cs.currentRArmIndex = findIndexByIDGeneric(cs.rArmPartsList, target.RightArmID, func(p *game.PartDefinition) string { return p.ID })
	cs.currentLArmIndex = findIndexByIDGeneric(cs.lArmPartsList, target.LeftArmID, func(p *game.PartDefinition) string { return p.ID })
	cs.currentLegsIndex = findIndexByIDGeneric(cs.legsPartsList, target.LegsID, func(p *game.PartDefinition) string { return p.ID })
...
func (cs *CustomizeScene) createPartSelectionRow(parent *widget.Container, label game.CustomizeCategory) *widget.Button {
	res := cs.resources
...
func (cs *CustomizeScene) changeSelection(label game.CustomizeCategory, direction int) {
	target := cs.playerMedarots[cs.currentTargetMedarotIndex]
	var listSize int
...
func (cs *CustomizeScene) updateStatus(id string) {
	...
	if partDef, found := cs.resources.GameDataManager.GetPartDefinition(id); found {
		...
		if partDef.Type == game.PartTypeLegs {
...
func (cs *CustomizeScene) getCurrentID(label game.CustomizeCategory) string {
	switch label {
	case game.CustomizeCategoryMedal:
...
func (cs *CustomizeScene) getCurrentName(label game.CustomizeCategory) string {
	switch label {
	case game.CustomizeCategoryMedal:
		if len(cs.medalList) > 0 && cs.currentMedalIndex < len(cs.medalList) {
