...
func NewPlaceholderScene(res *SharedResources, manager SceneManagerChanger, message string) *PlaceholderScene {
	p := &PlaceholderScene{
		resources: res,
		manager:   manager,
	}
	...
	subText := widget.NewText(
		widget.TextOpts.Text("クリックしてタイトルに戻る", res.Font, res.UIConfig.Colors.Gray),
	)
...
func (p *PlaceholderScene) Draw(screen *ebiten.Image) {
	screen.Fill(p.resources.UIConfig.Colors.Background)
	p.ui.Draw(screen)
}

func (p *PlaceholderScene) Layout(outsideWidth, outsideHeight int) (int, int) {
	return p.resources.UIConfig.Screen.Width, p.resources.UIConfig.Screen.Height
}
