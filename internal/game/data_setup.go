...
func CreateMedarotEntities(world donburi.World, gameData *GameData, playerTeam TeamID, gdm *GameDataManager) {
	for _, loadout := range gameData.Medarots {
		...
		for slot, partID := range partIDMap {
			partDef, defFound := gdm.GetPartDefinition(partID)
			if defFound {
		...
		medalDef, medalFound := gdm.GetMedalDefinition(loadout.MedalID)
		if medalFound {
