...
// GameOverTransitionGameEvent は、ゲームオーバー画面から次のシーンへの遷移を要求するイベントです。
type GameOverTransitionGameEvent struct{}

func (e GameOverTransitionGameEvent) isGameEvent() {}
...
// AnimationResultViewModel は、UIがアニメーションを描画するために必要なActionResultのサブセットです。
type AnimationResultViewModel struct {
	ActingEntry    *donburi.Entry
	TargetEntry    *donburi.Entry
	OriginalDamage int
	StartTime      int
...
// AnimationManager はアニメーションの状態を管理するためのインターフェースです。
// これにより、UIパッケージがBattleパッケージに直接依存するのを防ぎます。
type AnimationManager interface {
	SetAnimation(anim *ActionAnimationData)
	IsAnimationFinished(tick int) bool
	ClearAnimation()
	GetCurrentAnimationResult() AnimationResultViewModel
}
